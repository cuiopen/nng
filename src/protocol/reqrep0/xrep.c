...
xrep0_pipe_init(void **pp, nni_pipe *pipe, void *s)
{
	...
	// XXX: sendq on pipes should be used instead of the queue on
	// the send message.  We can have send simply bypass the upper
	// write queue -- there is no point in using that to apply
	// backpressure.  The whole backpressure with device fan out
	// needs some investment in thought.
	...
	// but it takes some time.  (In the future we might want to find
	// a way to throttle back the receive rate on that pipe, so that
	// we don't accept requests faster than the requester can handle
	// replies!  This would be a significant effort, but might be worth
	// it for very large scale systems.)  The way to handle this is
	// to have the pipe not signal that its ready to receive from
	// while it is busy sending.
